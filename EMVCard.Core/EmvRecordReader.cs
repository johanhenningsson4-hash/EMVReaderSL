...
namespace EMVCard
{
    ...
    public class EmvRecordReader
    {
        ...
        /// <summary>
        /// Read all records specified in AFL list asynchronously.
        /// </summary>
        /// <param name="aflList">List of AFL entries</param>
        /// <param name="cardData">Card data object to populate</param>
        /// <returns>True if at least one record was read successfully</returns>
        public Task<bool> ReadAFLRecordsAsync(List<(int sfi, int start, int end)> aflList, EmvDataParser.EmvCardData cardData)
        {
            return Task.Run(() => ReadAFLRecords(aflList, cardData));
        }

        /// <summary>
        /// Read a specific record from the card asynchronously.
        /// </summary>
        /// <param name="sfi">Short File Identifier</param>
        /// <param name="record">Record number</param>
        /// <param name="cardData">Card data object to populate</param>
        /// <returns>True if record read successfully</returns>
        public Task<bool> ReadRecordAsync(int sfi, int record, EmvDataParser.EmvCardData cardData)
        {
            return Task.Run(() => ReadRecord(sfi, record, cardData));
        }

        /// <summary>
        /// Try reading common SFI/Record combinations when AFL is not available asynchronously.
        /// </summary>
        /// <param name="cardData">Card data object to populate</param>
        /// <returns>True if at least one record was read</returns>
        public Task<bool> TryReadCommonRecordsAsync(EmvDataParser.EmvCardData cardData)
        {
            return Task.Run(() => TryReadCommonRecords(cardData));
        }
